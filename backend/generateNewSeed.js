...
const generateProducts = () => {
  ...
  categories.forEach(cat => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const prompt = encodeURIComponent(`Premium modern minimalist home decor ${adjective} ${noun}, pure white background, studio lighting, hyperrealistic, 8k resolution`);
      const seedBase = Math.floor(Math.random() * 999999);
      
      const mainImageUrl = `https://picsum.photos/seed/${seedBase}/800/800`;
      const image2Url = `https://picsum.photos/seed/${seedBase + 1}/800/800`;
      const image3Url = `https://picsum.photos/seed/${seedBase + 2}/800/800`;
